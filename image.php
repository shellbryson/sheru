    ...
    ?>
      ...
      <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
        ...
        <nav id="image-navigation" class="navigation image-navigation">
          <div class="nav-links">
            <div class="nav-previous"><?php previous_image_link( false, __( 'Previous Image', 'sheru' ) ); ?></div>
            <div class="nav-next"><?php next_image_link( false, __( 'Next Image', 'sheru' ) ); ?></div>
          </div><!-- .nav-links -->
        ...
        <div class="entry-content">
          ...
          <div class="entry-attachment">
            <?php
              /**
               * Filter the default sheru image attachment size.
               *
               * @since Twenty Sixteen 1.0
               *
               * @param string $image_size Image size. Default 'large'.
               */
              $image_size = apply_filters( 'sheru_attachment_size', 'large' );

              echo wp_get_attachment_image( get_the_ID(), $image_size );
            ?>

            <?php sheru_excerpt( 'entry-caption' ); ?>
          ...
          <?php
            ...
            if ( $metadata ) {
              printf( '<span class="full-size-link"><span class="screen-reader-text">%1$s </span><a href="%2$s">%3$s &times; %4$s</a></span>',
                esc_html_x( 'Full size', 'Used before full size attachment link.', 'sheru' ),
