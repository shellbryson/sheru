 ...
 *
 * Create your own sheru_entry_meta() function to override in a child theme.
 *
 * @since Twenty Sixteen 1.0
 */
function sheru_entry_meta() {
	if ( 'post' === get_post_type() ) {
		$author_avatar_size = apply_filters( 'sheru_author_avatar_size', 49 );
	...
	if ( current_theme_supports( 'post-formats', $format ) ) {
		printf( '<span class="entry-format">%1$s<a href="%2$s">%3$s</a></span>',
			sprintf( '<span class="screen-reader-text">%s </span>', _x( 'Format', 'Used before post format.', 'sheru' ) ),
	...
	if ( ! is_singular() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'sheru' ), get_the_title() ) );
		echo '</span>';
	}
}
endif;
 ...
 *
 * Create your own sheru_entry_date() function to override in a child theme.
 *
 * @since Twenty Sixteen 1.0
 */
function sheru_entry_date() {
	...
	printf( '<span class="posted-on"><span class="screen-reader-text">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
		_x( 'Posted on', 'Used before publish date.', 'sheru' ),
...
function sheru_excerpt_more() {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		...
		sprintf( __( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'sheru' ), get_the_title( get_the_ID() ) )
	);
	return ' &hellip; ' . $link;
}
